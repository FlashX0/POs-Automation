...
export class UserService {
  ...
  public static async seedAllRequiredUsers() {
    try {
      ...
      // Wrapped in a try-catch so if Supabase is unavailable, the server won't hang
      try {
        ...
        console.warn("[Seeder] Supabase admin sync error (continuing without purging):", adminErr.message);
      }
    } catch (err: any) {
  ...
  public static async login(email: string, password: string): Promise<any> {
    ...
    try {
      ...
      console.warn("[Login] User seeding failed/skipped, continuing login process.");
    }
  ...
  public static async verifySession(email: string): Promise<any> {
    ...
    if (adminClient) {
      try {
        ...
        if (error) {
          console.error("[verifySession] Supabase direct user search error:", error.message);
        } else if (data) {
          matchedUser = {
            id: data.id,
            name: data.name,
  ...
  public static async createUser(name: string, email: string, password: string, role: string, allowed_departments: string[]) {
    ...
    try {
      ...
      const db = getDb();
      db.users = db.users || [];
      const localExists = db.users.some((u: any) => u.email && u.email.toLowerCase() === lowerEmail);
      ...
      if (!supabaseAdmin) {
        ...
        throw new Error("فشل إنشاء الحساب: مفتاح الخدمة SUPABASE_SERVICE_ROLE_KEY غير مهيأ على السيرفر.");
      }
      ...
      try {
        ...
        console.warn("[Supabase DB] profiles insert warning:", profileErr.message);
      }
  ...
  public static async updateUser(id: string, email: string, name: string, role: string, status: string, password?: string, newEmail?: string, allowed_departments?: string[]) {
    try {
      ...
      if (userIndex === -1) {
        userIndex = db.users.findIndex((u: any) => u.email && u.email.toLowerCase() === lowerEmail);
      }
      ...
      if (isSystemAccount) {
        if (role && role !== db.users[userIndex].role) {
      ...
      if (newEmail && newEmail.toLowerCase().trim() !== lowerEmail) {
        ...
        const isTaken = db.users.some((u: any) => u.email && u.email.toLowerCase() === lowerNewEmail);
        if (isTaken) {
      ...
      const targetUserId = id || db.users[userIndex].id;
      ...
      if (supabaseAdmin && targetUserId) {
        try {
          ...
          await supabaseAdmin.auth.admin.updateUserById(targetUserId, authUpdateObj);
        } catch (ex: any) {
          console.error("[Supabase Admin Auth] Exception updating user auth:", ex.message);
        }
      }
  ...
  public static async deleteUser(id: string, email: string) {
    try {
      ...
      const targetUserId = id || (userToDelete ? userToDelete.id : "");
      const userEmailForDeletion = lowerEmail || (userToDelete ? userToDelete.email?.toLowerCase().trim() : "");
      ...
      if (userEmailForDeletion) {
        try {
          ...
          await supabaseAdmin.from('users').delete().eq('email', userEmailForDeletion);
        } catch (e: any) {
          console.warn("Could not delete from profiles/users table:", e.message);
        }
      }
