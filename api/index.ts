...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    // Note: In Vercel serverless runs, these static serving handlers are handled at the CDN level via vercel.json.
    // In standalone containers, these serve the built assets.
  ...
  // Only bind port on standalone dev/container environments. Vercel routes handlers automatically.
  if (!process.env.VERCEL) {
    app.listen(PORT, "0.0.0.0", () => {
      console.log(`Server running on http://localhost:${PORT}`);
      checkForUpcomingDueDates();
      setInterval(checkForUpcomingDueDates, 24 * 60 * 60 * 1000);
    });
  } else {
    // Instant trigger notifications for Vercel cold boot checks
    checkForUpcomingDueDates();
  }
}
...
// Export the app for Vercel Serverless runtime compatibility
export default app;
