...
// For extractDataFromDocument (lines ~670-740)
const extractDataRegex = /response = await ai\.models\.generateContent\(\{\s*model: currentModel,\s*contents: \{ parts: \[documentPart, textPart\] \},\s*config: \{\s*systemInstruction: systemInstruction \+ "\\n\\nCRITICAL RULE: Do NOT hallucinate numbers\. If unsure, output 0\. Strictly match the JSON schema\.",\s*responseMimeType: "application\/json",\s*responseSchema: \{[\s\S]*?\}\s*\}\s*\}\s*\}\);\s*break;/;

const extractDataReplace = `if (currentModel.includes('gemini')) {
...
code = code.replace(extractDataRegex, extractDataReplace);

// For extractFinancialFile (lines ~960-980)
const financialFileRegex = /response = await ai\.models\.generateContent\(\{\s*model: currentModel,\s*contents: \{ parts: \[documentPart, textPart\] \},\s*config: \{\s*systemInstruction: systemInstruction,\s*responseMimeType: "application\/json",\s*responseSchema: \{\s*type: Type\.OBJECT,\s*properties: schemaProperties,\s*required: requiredFields\s*\}\s*\}\s*\}\);\s*break;/;

const financialFileReplace = `if (currentModel.includes('gemini')) {
          response = await ai.models.generateContent({
            model: currentModel,
            contents: { parts: [documentPart, textPart] },
            config: {
...
code = code.replace(financialFileRegex, financialFileReplace);
